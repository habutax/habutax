...
class Form8889(Form):
    ...
    def __init__(self, **kwargs):
        ...
        inputs = [
            ...
            BooleanInput('hdhp_plan_family', description=f'Were you covered under a *family* high-deductible health plan (HDHP) during 2023 (as opposed to self-only coverage)? If {you} were covered, or considered covered, by a self-only high-deductible health plan (HDHP) and a family HDHP at different times during the year, answer "yes" only if the family plan was in effect for a longer period. If {you} were covered by both a self-only HDHP and a family HDHP at the same time, {you} are treated as having family coverage during that period. If, on the first day of the last month of {your} tax year (December 1 for most taxpayers), {you} had family coverage, answer "yes". Were you covered under a high-deductible *family* plan during 2023?'),
            FloatInput('hsa_contributions', description=f'Enter HSA contributions {you} made for 2023 (or those made on {your} behalf), including those made from January 1, 2023, through April 15, 2023, that were for 2023. Do not include employer contributions, contributions through a cafeteria plan, or rollovers. See instructions for Form 8889, line 2.'),
            BooleanInput('age_under_55', description=f'Were {you} under the age of 55 at the end of 2023?'),
            BooleanInput('hsa_full_year', description=f'Did {you} have a HDHP medical plan for the first day of every month during the 2023 year?'),
            FloatInput('archer_msa', description=f'Enter the amount {you} and {your} employer contributed to {your} Archer MSAs for 2023 from Form 8853, lines 1 and 2. If you or your spouse had family coverage under an HDHP at any time during 2023, also include any amount contributed to {your_spouses} Archer MSAs'),
            ...
            FloatInput('employer_contribution', description=f'Employer contributions made to {your} HSAs for 2023? NOTE: This number should match the sum of all {your} your forms W-2, box 12, code W.'),
        ]

        def calc_contribution_max(self, i, v):
            if i['age_under_55'] and i['hsa_full_year']:
                return self.threshold('hsa_family_contribution_limit') if v['1'] else self.threshold('hsa_individual_contribution_limit')
            else:
        ...
        pdf_fields = [
            ...
            TextPDFField('topmostSubform[0].Page1[0].f1_3[0]', '2'),
            TextPDFField('topmostSubform[0].Page1[0].f1_4[0]', '3'),
            TextPDFField('topmostSubform[0].Page1[0].f1_5[0]', '4'),
            TextPDFField('topmostSubform[0].Page1[0].f1_6[0]', '5'),
            TextPDFField('topmostSubform[0].Page1[0].f1_7[0]', '6'),
            TextPDFField('topmostSubform[0].Page1[0].f1_8[0]', '7'),
            TextPDFField('topmostSubform[0].Page1[0].f1_9[0]', '8'),
